...
#Gráficos de la confianza en otras regiones
...
#Gráfico de la variable de confianza sobre el gobierno
...
#Gráficos de la variable del miedo a perder el trabajo

#Gráficos por region sierra y costa
...
cf_r4_2013 <- cf_r_4 %>% 
  ...
  geom_errorbar(aes(ymin = mean - 1.96*se, ymax = mean + 1.96*se), width = 0.1) +
  geom_text(aes(label = scales::percent(mean, digits =1)), 
            color = "grey20", size = 3.2, vjust = -1.2) +
  labs(title = "2013", x = "", y = "Porcentaje de personajes temen perder su empleo (%)") + 
...
cf_r4_2018 <- cf_r_4 %>% 
  ...
  geom_errorbar(aes(ymin = mean - 1.96*se, ymax = mean + 1.96*se), width = 0.1) +
  geom_text(aes(label = scales::percent(mean, digits = 1)), 
            color = "grey20", size = 3.2, vjust = -1.5) +
  labs(title = "2018", x = "", y = "") +
  scale_y_continuous(labels = scales::percent_format(acurracy=1))+
  scale_fill_manual(values = c("lightcoral", "lightblue","green"))+
  theme_minimal()+
...
combined_plot_4 <- cf_r4_2013 + cf_r4_2018 +
  ...
  plot_annotation(title = "Miedo de los ecuatorianos a perder su empleo",
                  subtitle = "¿Hasta qué punto le preocupan las siguientes situaciones? 
  Perder mi trabajo o no encontrar trabajo",
...
ggsave(combined_plot_4, filename = "figures/gráfico_miedo_perder_empleo.png", width = 25, height = 15, units = "cm")

#---------------------------------------------------------------------------------
# Analisis de la variable confianza en la policia
cf_r_5 <- wvs_c %>% 
  group_by(ano, region) %>% 
...
df_graficos <-
  ...
  select(ano,
         region, 
         starts_with(c("mean", "se"))) %>% 
  rename(mean_otrareg = mean_otra_reg,
         se_otrareg = se_otra_reg) %>% 
  pivot_longer(cols = -c(ano,region),
               names_to = c("indicador", "variable"),
               names_sep = "_",
               values_to = "valor") %>% 
  pivot_wider(names_from = "indicador",
              values_from = "valor") %>% 
  mutate(ano = zap_labels(ano),
         confianza_from = case_when(variable == 'cf' ~ 'Confianza en la Familia',
                                    variable == 'otrareg' ~ 'Confianza en la otra Región',
                                    variable == 'gob' ~ 'Confianza en el Gobierno',
                                    variable == 'police' ~ 'Confianza en la Policía')
  )


caption_confianza_grafico_conjunto <- 
  "Las cifras representan en porcentaje la confianza que tienen las personas en los diferentes indicadores.\nLas barras representan intervalos de confianza del 95%. Fuente: Encuesta Mundial de Valores (WVS) olas 2013 y 2018."
...
grafico_2013_por_region <-
  df_graficos %>%
  filter(ano == 2013) %>% 
  ggplot(aes(x = variable, y = mean, fill = confianza_from)) +
  geom_col(position = position_dodge(), color = "black") +
  facet_wrap(~ region) +
  geom_errorbar(aes(ymin = mean - 1.96 * se, 
                    ymax = mean + 1.96 * se),
                position = position_dodge()) +
  geom_text(aes(label = scales::percent(mean, accuracy = 1), group = confianza_from),
  ...
  labs(title = '2013',
       x = "",
       y = "Porcentaje de personajes que denotan confianza (%)",
       fill = "") + 
  theme_minimal() +
  theme(axis.text.x = element_blank(),
        legend.position = "none",
        plot.background = element_rect(fill = "white", color = NA),
        panel.border = element_rect(colour = "black", fill = NA, linewidth = 0.5, linetype = "solid"),
...
grafico_confianza_por_region <- 
  ...
  plot_annotation(title = "Confianza en diferentes indicadores por región",
                  subtitle = "¿Podría decirme para cada uno si confía completamente, algo, poco o nada
    en las personas o instituciones de este grupo?",
                  caption = caption_confianza_grafico_conjunto) & theme(legend.position = "bottom") +
  theme(plot.caption = element_text(hjust = 0))

ggsave(grafico_confianza_por_region, filename = "figures/grafico_confianza_por_region.png", width = 25, height = 15, units = "cm")
...
Gráfico_de_confianza_nacional<-c_nacional %>% 
  select(ano,
         region, 
         starts_with(c("mean", "se"))) %>% 
  rename(mean_otrareg = mean_otra_reg,
  ...
  ggplot(aes(x = ano, y = mean, fill = confianza_from)) +
  ...
  theme(axis.text.x = element_text(angle = 45, vjust = 0.7, hjust = 1),
        ...
        panel.grid.minor = element_blank(),
        axis.line = element_line("black"))
print(Gráfico_de_confianza_nacional)
ggsave(Gráfico_de_confianza_nacional, filename = "figures/grafico_confianza_nacional.png", width = 25, height = 15, units = "cm")
